...
class CrosswordCreator():
    ...
    def revise(self, x, y):
        ...
        for x_val in list(self.domains[x]):
            for y_val in self.domains[y]:
                print(x_val, 'x → Y', y_val)
                print(overlap)
                print(x_val[overlap[0]], ' == ', y_val[overlap[1]])
    ...
    def consistent(self, assignment):
        ...
        for var, word in assignment.items():
            if word not in self.domains[var]:  # Ensure word is valid
                return False
            elif len(word) != var.length:
                return False
        
        for (v1,v2) in self.crossword.overlaps.keys():
            if (temp := self.crossword.overlaps[v1,v2]) is None:
                continue
            (p1,p2) = temp
    ...
    def order_domain_values(self, var, assignment):
        ...
        for value in self.domains[var]:
            dic[value] = 0
            for (v1,v2) in self.crossword.overlaps.keys():
                if (temp := self.crossword.overlaps[v1,v2]) == None:
                    continue
                (p1,p2) = temp
                if v1 == var:
                    for value2 in self.domains[v2]: 
    ...
    def select_unassigned_variable(self, assignment):
        ...
        for var in self.crossword.variables:
            if var in assignment.keys():
                continue
            if lowest == None or len(self.domains[var]) < lowest:
