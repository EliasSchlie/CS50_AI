...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        for i in range(cell[0] - 1, cell[0] + 2):
            for j in range(cell[1] - 1, cell[1] + 2):
                ...
                # Ignore the cell itself & safe cells
        ...
        while new_knowledge:
            ...
            for s1 in self.knowledge:
                for s2 in self.knowledge:
                    if s1 == s2 or len(s2.cells.difference(s1.cells)) == 0:
                        continue
                    if s1.cells.issubset(s2.cells):
                        # if new sentence not already in knowledge
                        #print("KNOWLEDGE:")
                        for k in self.knowledge:
                            print(k)
                        #print("COMPARE WITH:")
                        s = Sentence(s2.cells.difference(s1.cells), s2.count - s1.count)
                        print(s)
    ...
    def clean_knowledge(self):
        ...
        for sentence in self.knowledge:
            #print(sentence)
            if (s := sentence.known_safes()):
                #print(s, " is safe")
                for saf in list(s):
                    self.mark_safe(saf)
            elif (m := sentence.known_mines()):
                #print(m, " is mines")
                for mine in list(m):
                    self.mark_mine(mine)
